...
激活函数一般都是非线性函数。没有激活函数的网络层都是线性函数（超平面），输出的是 $y$ 的“衍生物” $u=Activate^{-1}(y)$ （类似换元），只有经过激活函数后才是真正的输出 $y$。之所以用线性函数先拟合 $u$ ，是因为直接用非线性函数对 $y$ 拟合会带有很多待学习的参数，而激活函数一般是线性函数的复合函数，只含少量待学习参数。
