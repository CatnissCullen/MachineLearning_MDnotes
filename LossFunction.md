...
## MAE Loss (L1/ Pixel Loss) 及其相关损失

![image-20240913153920461](./img/image-20240913153920461.png)

### Identity Loss

***在一个分布内求各个样本的 L1 的期望***

![image-20240913154129632](./img/image-20240913154129632.png)



## MSE Loss (L2 loss)

MSE会计算模型预测值与真实值之间的平均平方差，衡量预测值与真实值之间的偏差。MSE的值越小，说明模型的预测效果越好。

用于**LSGAN**、**Diffusion的噪声预测**。
...
它通常用于分类问题。在分类问题中，我们试图预测一个离散的类别标签，如图像分类、文本分类等。交叉熵损失衡量的是模型预测的概率分布与真实的概率分布之间的距离，是**衡量模型预测概率与真实情况（概率为1或0）一致性**的一个度量。交叉熵损失值越小，说明模型的预测效果越好。
...
用于**Vanilla GAN**、**classifier guidance**。
...
-   ***似然函数：***给定样本点（真实分布）时，将估计的参数代入分布函数采样得到的样本中给定样本点出现的概率（估计分布和真实分布的重合概率）
-   ***概率函数：***给定分布参数时，某点在该分布中能出现的概率
...
**用正样本距离去惩罚负样本距离形成相对距离**

**L = max(||xa - xp||^2 - ||xa - xn||^2 + margin, 0)**



## Contrastive Loss

***要求没有三元组损失那么严格，因为计算负样本距离是与正样本距离相独立的，没有去用正样本距离去惩罚负样本距离形成相对距离***

**L(x1,x2,y) = y * D(x1,x2) + (1-y) * max(margin - D(x1,x2), 0)**

-   其中: x1,x2 是两个输入样本
-   y 是标签，当x1和x2为正样本对时y=1，否则为0 
-   D(x1, x2) 是x1和x2之间的距离度量
-   margin是设定的边际参数当样本对为正样本对时，损失函数会减小其距离

当样本对为负样本对且距离小于margin时，损失函数会增大其距离
...
**比交叉熵更关注近似分布 Q 到真实分布 P 的相对距离**

-   **交叉熵：衡量 Q 中样本标签的编排与 P 中的差距**
-   **KL 散度：衡量 Q 与 P 相比损失的信息量**

**KL 散度比交叉熵多减了一个 P 的熵** 
$$
\mathcal{D}_{KL}(P||Q) = \sum_xP(x)\log\frac{P(x)}{Q(x)}
$$
![image-20240913155851819](./img/image-20240913155851819.png)



## Wasserstein Distance

***专用于概率分布生成模型，求两==<u>分布</u>==间的差距（生成分布相对实际分布\<抽样获得>的Loss）***
...
$$
StyleSim. = -\sum||Gram_{Input}(X_t,t)-Gram_{Ref}(R_t,t)||_F^2
$$
