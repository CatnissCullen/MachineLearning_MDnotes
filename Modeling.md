...
激活函数一般都是非线性函数。没有激活函数的网络层都是线性函数（超平面），输出的是 $y$ 的“衍生物” $u=Activate^{-1}(y)$ （类似换元），只有经过激活函数后才是真正的输出 $y=Activate(u)$。之所以用线性函数先拟合 $u=W\cdot X+\beta$ ，是因为（1）线性函数比非线性函数本身就更容易优化；（2）直接用非线性函数对 $y$ 拟合会带有很多待学习的参数，而激活函数一般是线性函数的复合函数，只含少量待学习参数。
...
-   **改进模型：使用带有非线性激活函数的多层线性函数（深度神经网络），而非单层非线性函数**，这样使用反向传播分别更新各层（各维度）的参数，就比单层的很多参数都在同一维度更容易训练
...
-   **第一种参数耦合——实际不相关的参数被放在同层网络中一起优化，被迫协同更新：**

    现实生活中问题中的两个因素其实是不相关的，但是建模时没有考虑到这点，没有把它们区分成两个独立的因素，而是混合在同一个函数的参数中进行优化，使得并不能独立优化两个参数，导致原本应在空间中相互垂直的向量上各自调整的参数，现在被处理为不互相垂直的、一个改变就会牵扯到另一个的改变的两个关联的参数，进而不能同时获得两个参数各自对模型输出有最优贡献的值

-   **第二种参数耦合——实际为单个的参数被建模成多个参数在网络中优化，被迫增加优化难度：**

    原本问题只受到一个参数影响，但是建模时将问题复杂化了，设置了多个参数，这多个参数原本是可以作为单个参数来优化的现在却需要各自优化并且又受到相互牵扯，进而增加了模型优化难度。
